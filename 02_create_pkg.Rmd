...
- These codes create a new package `urosconf` and a Rstudio project
```{r, eval = FALSE}
usethis::create_package(path = "~/urosconf")
```

- if Rstudio is available, it switches to the new project
- Note: one may change defaults of the `DESCRIPTION` file
```{r, eval = FALSE}
usethis::create_package(path = "~/urosconf", fields = list(
  `Authors@R` = 'person(
...
cat(readLines("files/DESCRIPTION_1"), sep = "\n")
```
